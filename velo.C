...
double FoilMaterial::Segment::p(unsigned int idx, double z) {
  if (idx >= fncs.size()) return 0;
  if (mths[idx] == 1 && z < spls[idx].GetXmax() && z > spls[idx].GetXmin())
    return spls[idx].Eval(z);
  return fncs[idx].Eval(z);
}
...
TLorentzVector FoilMaterial::Intersect::intersect(int halfIn) {
  ...
  if (s > 0.01) return TLorentzVector(0, 0, 0, 1);
  else return TLorentzVector(vx + fx*t, vy + fy*t, vz + fz*t, s);
}
...
(double vxIn, double vyIn, double vzIn, double fxIn, double fyIn, double fzIn,
 int halfIn) {
  double f(sqrt(fxIn*fxIn + fyIn*fyIn + fzIn*fzIn)); f = f == 0 ? 1 : f;
  vx = vxIn; vy = vyIn; vz = vzIn; fx = fxIn/f; fy = fyIn/f; fz = fzIn/f;
  if (halfIn == 0) {
    TLorentzVector sl(intersect(-1)), su(intersect(1));
    if (sl.T() < su.T()) {sl.SetT(-1); return sl;}
    else if (sl.T() > su.T()) {su.SetT(1); return su;}
    else if (sl.T() != 1) {sl.SetT(-1); return sl;}
    else return TLorentzVector(0, 0, 0, 0);
  }
  TLorentzVector s(intersect(halfIn));
  if (s.T() != 1) {s.SetT(halfIn); return s;}
  else return TLorentzVector(0, 0, 0, 0);
}
...
// Return a parameter of the foil function.
double FoilMaterial::p(unsigned int idx, double z, int half) {
  vector<Segment> *fzs = half < 0 ? &flzs : &fuzs;
  for (int seg = 0; seg < (int)fzs->size(); ++seg)
    if ((*fzs)[seg].valid(z)) return (*fzs)[seg].p(idx, z);
  return 0;
}
